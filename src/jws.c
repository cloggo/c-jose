...
NAPI_METHOD(c_jose_jws_ver) {
  ...
  bool all = false;
  status = napi_get_value_bool(env, argv[3], &all);
  assert(status == napi_ok);

  bool out = jose_jws_ver(NULL, (json_t*)jws, (json_t*)sig, (json_t *)jwk, all);
...
NAPI_METHOD(c_jose_jws_ver_io) {
  ...
  jose_io_t* out = jose_jws_ver_io(NULL, (json_t*)jws, (json_t*)sig, (json_t *)jwk, all);
